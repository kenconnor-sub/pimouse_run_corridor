...
class WallStopAccelTest(unittest.TestCase):
	...
	def test_io(self):
		...
		self.assertTrue(True, "can't stop")

		left, right = self.set_and_get(0,5,1000,0) #side direction is not a trigger of stop
		self.assertTrue(True, "stop wrongly by side sensors")
		...
		self.assertTrue(0 < left == right, "curve wrongly left={} right={}".format(left,right))
